...
// Simple deep equal implementation to replace lodash
...
export function MoveProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // Initialize edit mode state from window if available
  ...
  useEffect(() => {
    if (pendingMove && typeof window !== 'undefined') {
  ...
  const moveNode = ({ itemId, sourceContainerId, destContainerId, newIndex, oldIndex }: {
    ...
    setNodeRegistry(prev => {
      const srcId = sourceContainerId.toString();
      const dstId = destContainerId.toString();
      ...
      const idx = srcChildren.indexOf(itemIdStr);
      if (idx === -1) return prev;

      srcChildren.splice(idx, 1);
      destChildren.splice(newIndex, 0, itemIdStr);

      nextContainerToChildren[srcId] = srcChildren;
      nextContainerToChildren[dstId] = destChildren;

      const nextItemToParent =
        srcId === dstId ? prev.itemToParent : { ...prev.itemToParent, [itemIdStr]: dstId };
